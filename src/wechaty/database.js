...
// 插入数据函数
export async function insertData(datetime, userId, textContent) {
  let insertSql = 'INSERT INTO user_texts (datetime, user_id, text_content) VALUES (?, ?, ?)';
  await new Promise((resolve, reject) => {
    db.run(insertSql, [datetime, userId, textContent], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve();
...
export async function queryRecentTexts(userId, currentTime) {
  ...
  let selectSql = `
    SELECT datetime, text_content, ROW_NUMBER() OVER (ORDER BY datetime) as row_num
    FROM user_texts 
  ...
  rows.forEach((row, index) => {
    msgs.push({role: role, content: row.text_content});
